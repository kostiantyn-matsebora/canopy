...
- **[Primitives](reference/PRIMITIVES.md)** — `IF`, `SWITCH`, `FOR_EACH`, `ASK`, `SHOW_PLAN`, `VERIFY_EXPECTED`, etc. Auto-mirrored from `skills/canopy-runtime/references/framework-ops.md` so the spec on this site is exactly what the runtime executes.
