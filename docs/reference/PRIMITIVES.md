...
> **Auto-generated mirror.** Canonical source: [`skills/canopy-runtime/references/framework-ops.md`](https://github.com/kostiantyn-matsebora/claude-canopy/blob/master/skills/canopy-runtime/references/framework-ops.md). Edits go to the canonical file; this page is regenerated by `scripts/sync-runtime-docs.py`.
...
**Marker-based children:** when a child is `**OP_NAME** << ... >> ...` (bold around the op name) and the resolved op definition carries `> **Subagent.** Output contract: <schema>`, the runtime dispatches the op's body as a subagent with the bound `<<` values as inputs and binds the schema-shaped result. Plain (un-bold) op-call children run inline. See `skill-resources.md` → `## Subagent dispatch` for the full contract.
